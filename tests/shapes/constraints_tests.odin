...
make_distance_constraint :: proc(
    ...
    return shapes.Shapes_Constraint{
        ...
        do_apply = true,
        depend_on = depend_on,
        restriction = shapes.Vector3{req_len, 0, 0},
    }
}
...
rotate_around_axis_preserves_length :: proc(t: ^testing.T) {
    ...
    input := shapes.Vector3{3, 4, 0}
    output := shapes.rotate_around_axis(input, axis, math.PI / 2)
...
resolve_constraint_targets_supports_child_offset :: proc(t: ^testing.T) {
    ...
    points[0] = shapes.Shapes_Point{
        child_point_head = 1,
        child_count = 3,
    }

    points[1] = shapes.Shapes_Point{next_child_point = 2}
    points[2] = shapes.Shapes_Point{next_child_point = 3}
    points[3] = shapes.Shapes_Point{}

    constraint := shapes.Shapes_Constraint{
        kind = .Distance,
        on_point = 0,
        child_offset = 1,
        do_apply = true,
    ...
    targets, ok := shapes.resolve_constraint_targets(&constraint, &points)
