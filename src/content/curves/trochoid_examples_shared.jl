...
const GeometryMode = Mode == OdinJuliaBridge.TROCHOID_EXTERNAL ?
    EuclidGeometry.TrochoidExternal : EuclidGeometry.TrochoidInternal
...
"""Immutable native identity and cycle timing for one Trochoid construction."""
...
function instrument_pose(parameter::Real, elevation::Real)
    pose = EuclidGeometry.trochoid_tool_pose(Center, GeometryMode,
        FixedRadius, RollingRadius, parameter)
    tracer = EuclidGeometry.trochoid_point(Center, GeometryMode,
        FixedRadius, RollingRadius, TracerDistance, parameter)
    pose.rolling_center[3] = Float32(elevation)
    tracer[3] = Float32(elevation)
    return pose.rolling_center, tracer
end

"""Place the Trochoid guide and compass at one parameter and elevation."""
...
function reset_cycle_state(state_ptr::Ptr{Cvoid}, state::AnimationState)
    ...
    OdinJuliaBridge.set_trochoid_tool_geometry(state_ptr; mode=Mode,
...
"""Create the persistent Trochoid and initialize its first cycle."""
function initialize(state_ptr::Ptr{Cvoid})
    curve = create_curve(state_ptr, Center;
...
"""Advance the synchronized guide, compass, and persistent Trochoid."""
...
"""Dispatch one bridge-stable lifecycle operation for this Trochoid."""
