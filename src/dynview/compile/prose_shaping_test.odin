...
Prose_Shaping_Rebuild_Fixture :: struct {
    ...
    workspace: [8]fontmodel.Shaped_Glyph,
}
...
prose_shaping_test_service :: proc(
    workspace: []fontmodel.Shaped_Glyph) -> Document_Prose_Shaping_Service {
    ...
    result.fonts[int(fontmodel.Font_Key.Bold)] = {.Bold, 11, 25}
    return result
}
...
prose_shaping_rebuild_fixture_init :: proc(
    ...
    fixture^.inlines = {
        {kind = .Text, text_offset = 0, text_count = 1,
            font_flags = i32(fontmodel.Font_Variant_Flags.Regular)},
        {kind = .Space, text_offset = 1, text_count = 1,
            font_flags = i32(fontmodel.Font_Variant_Flags.Regular)},
        {kind = .Text, text_offset = 2, text_count = 1,
            font_flags = i32(fontmodel.Font_Variant_Flags.Bold)},
    }
    text: string = "A B"
...
prose_shaping_test_builder :: proc(
    t: ^testing.T, arena: ^storage.Arena_Owner) -> Document_Shaped_Builder {
...
prose_shaping_test_append :: proc(
    inline_index: int,
    glyphs: []fontmodel.Shaped_Glyph) -> Document_Shaped_Append {
...
document_prose_shaping_seals_complete_records :: proc(t: ^testing.T) {
    ...
    glyphs := [1]fontmodel.Shaped_Glyph{{glyph_id = 3, x_advance = 1024}}
    testing.expect_value(t, document_shaped_builder_append(
        &builder, prose_shaping_test_append(0, glyphs[:])),
        storage.Bounded_Builder_Status.Ok)
    cache := new(dynviewmodel.Dynview_Compile_Cache, allocator)
    generations: [fontmodel.FONT_KEY_COUNT]u64
    generations[int(fontmodel.Font_Key.Regular)] = 7
...
document_prose_shaping_rejects_stale_generation :: proc(t: ^testing.T) {
    ...
    defer storage.arena_owner_destroy(&arena)
    allocator := storage.arena_owner_allocator(&arena)
    glyphs := [1]fontmodel.Shaped_Glyph{{glyph_id = 3}}
    _ = document_shaped_builder_append(
        &builder, prose_shaping_test_append(0, glyphs[:]))
    cache := new(dynviewmodel.Dynview_Compile_Cache, allocator)
    generations: [fontmodel.FONT_KEY_COUNT]u64
    generations[int(fontmodel.Font_Key.Regular)] = 8
...
document_prose_shaping_rejects_glyph_overflow :: proc(t: ^testing.T) {
    ...
    builder := prose_shaping_test_builder(t, &arena)
    defer storage.arena_owner_destroy(&arena)
    builder.glyphs.count = dynviewmodel.DYNVIEW_MAX_DOCUMENT_SHAPED_GLYPHS
    glyphs := [1]fontmodel.Shaped_Glyph{{glyph_id = 3}}

    status := document_shaped_builder_append(
        &builder, prose_shaping_test_append(0, glyphs[:]))

    testing.expect_value(t, status, storage.Bounded_Builder_Status.Limit_Exceeded)
...
document_prose_shaping_rejects_run_overflow :: proc(t: ^testing.T) {
    ...
    builder := prose_shaping_test_builder(t, &arena)
    defer storage.arena_owner_destroy(&arena)
    builder.runs.count = dynviewmodel.DYNVIEW_MAX_DOCUMENT_SHAPED_RUNS
    glyphs := [1]fontmodel.Shaped_Glyph{{glyph_id = 3}}
...
document_prose_shaping_measures_semantic_inlines_deterministically :: proc(
    ...
    testing.expect(t, storage.arena_owner_init(&runtime_arena))
    testing.expect(t, storage.arena_owner_init(&cache_arena))
    defer storage.arena_owner_destroy(&runtime_arena)
    defer storage.arena_owner_destroy(&cache_arena)
    runtime_allocator := storage.arena_owner_allocator(&runtime_arena)
    runtime := new(dynviewmodel.Dynview_System, runtime_allocator)
    ...
    first_runs := [3]dynviewmodel.Dynview_Document_Shaped_Run{}
    first_glyphs := [3]fontmodel.Shaped_Glyph{}
    ...
    testing.expect_value(t, first_status, storage.Bounded_Builder_Status.Ok)
    testing.expect_value(t, second_status, storage.Bounded_Builder_Status.Ok)
    for index in 0..<3 {
    ...
    testing.expect_value(t, first_runs[2].requested_font_key, fontmodel.Font_Key.Bold)
    testing.expect_value(t, first_runs[2].effective_font_key, fontmodel.Font_Key.Bold)
...
document_prose_shaping_rejects_invalid_cluster :: proc(t: ^testing.T) {
    glyphs := [1]fontmodel.Shaped_Glyph{{glyph_id = 2, cluster = 1}}
