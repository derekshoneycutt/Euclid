...
compiled_bytes_test_arena_init :: proc(
    ...
    testing.expect(t, core.arena_owner_init(arena, 256*uint(mem.Kilobyte)))
}
...
compiled_bytes_publish_sealed_plain_and_copy_payloads :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, status, DYNVIEW_STATUS_OK)
...
compiled_bytes_reject_incomplete_stream_without_publication :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, status, DYNVIEW_STATUS_ILLEGAL_STATE)
...
compiled_bytes_consume_published_content_views :: proc(t: ^testing.T) {
    ...
    commands := [3]core.Dynview_Command{
...
compiled_bytes_reject_plain_text_overflow_without_publication :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, core.bounded_byte_builder_init(
        &state.plain_text_builder, core.DYNVIEW_MAX_TEXT_BYTES, &arena),
        core.Bounded_Builder_Status.Ok)
    state.plain_text_builder.count = core.DYNVIEW_MAX_TEXT_BYTES

    status := append_compiled_byte(cache, &state, 'x')

    testing.expect_value(t, status, DYNVIEW_STATUS_OUT_OF_CAPACITY)
...
compiled_copy_blocks_reject_exact_limit_overflow :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, core.bounded_element_builder_init(
        &state.copy_block_builder, core.DYNVIEW_MAX_COMMANDS, &arena),
        core.Bounded_Builder_Status.Ok)
    state.copy_block_builder.count = core.DYNVIEW_MAX_COMMANDS
    allocator := core.arena_owner_allocator(&arena)
    cache := new(core.Dynview_Compile_Cache, allocator)
...
copy_hit_targets_reuse_capacity_across_frames :: proc(t: ^testing.T) {
    ...
    cache^.copy_blocks = []core.Dynview_Copy_Block{{block_id = 3, payload_len = 2}}
    cache^.copy_block_count = 1
    cache^.layout_items = []core.Dynview_Layout_Item{{block_id = 3, line_index = 0}}
    cache^.layout_item_count = 1
    cache^.layout_lines = []core.Dynview_Layout_Line{{row_span = 1}}
...
copy_hit_targets_reject_exact_limit_overflow :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, core.bounded_element_builder_init(
        &cache^.copy_hit_target_builder, core.DYNVIEW_MAX_COMMANDS, &arena),
        core.Bounded_Builder_Status.Ok)
    cache^.copy_hit_target_builder.count = core.DYNVIEW_MAX_COMMANDS
