...
layout_consume_text_run :: proc(
    ...
    if len(text) <= 0 {
        return dyncore.DYNVIEW_STATUS_OK, -1
    }
    ...
    placement_status := layout_prepare_style_placement(
        ...
        style,
        ctx^.font_size)
    if placement_status != dyncore.DYNVIEW_STATUS_OK {
        return placement_status, -1
    }

    max_cols := layout_max_cols(ctx^.cache)
    ascent, descent := dyncore.style_ascent_descent(style, ctx^.font_size)
...
//   Measure one nonempty wrapped span for layout publication.
layout_wrapped_span_metrics :: #force_inline proc(
    ctx: Text_Wrap_Context,
    span: dyncore.Wrapped_Text_Span) -> Wrapped_Line_Metrics {

    return {
        byte_start = span.line_start,
        byte_len = span.line_end - span.line_start,
        col_span = dyncore.text_codepoint_count_span(
            ctx.text, span.line_start, span.line_end),
        ascent = ctx.ascent,
        descent = ctx.descent,
    }
}
...
layout_wrap_one_segment :: proc(
    ...
    if available <= 0 {
        ...
        return start, dyncore.DYNVIEW_STATUS_OK
    }

    span := dyncore.next_wrapped_text_span(ctx.text, start, available)
    metrics := layout_wrapped_span_metrics(ctx, span)
    if metrics.col_span <= 0 || metrics.byte_len <= 0 {
        return start, dyncore.DYNVIEW_STATUS_OK
    }
    ...
    return span.next_start, dyncore.DYNVIEW_STATUS_OK
}
...
layout_wrap_text_run :: proc(ctx: Text_Wrap_Context) -> (i32, int) {
    ...
    return dyncore.DYNVIEW_STATUS_OK, last_line
}
...
layout_consume_math_block :: proc(
    ...
    if measure_status != dyncore.DYNVIEW_STATUS_OK {
...
math_block_layout_metrics :: proc(
    ...
    text_ascent, text_descent := dyncore.style_ascent_descent(style, ctx^.font_size)
...
layout_finalize_after_inline_if_full :: #force_inline proc(
    ...
    if state^.col < metrics.max_cols {
        return dyncore.DYNVIEW_STATUS_OK, state^.line_index
    }
    ...
    return dyncore.DYNVIEW_STATUS_OK, state^.line_index - 1
}
...
layout_build_context :: proc(
    ...
    base_style := dyncore.style_by_id(dyncore.DYNVIEW_STYLE_OUTPUT)
    base_ascent, base_descent := dyncore.style_ascent_descent(
        base_style, cache^.last_font_size)
    cells := layout_cell_metrics(cache, base_ascent, base_descent)
    state^ = Dynview_Layout_State{
        active_block_id = -1,
        active_block_kind = -1,
...
layout_handle_block_markers :: proc(
    ...
    if cmd.kind == .Begin_Block {
        ...
        if spacing_status != dyncore.DYNVIEW_STATUS_OK {
...
inline_draw_color :: #force_inline proc(
    style: dyncore.Dynview_Text_Style,
    item: dynviewmodel.Dynview_Layout_Item) -> dynviewmodel.Color {
...
layout_consume_commands :: proc(
    ...
    for cmd in commands {
        ...
        style := dyncore.style_by_id(cmd.style_id)
...
rebuild_layout_cache :: proc(
    ...
    commands := dyncore.command_buffer_commands(buffer)
    ...
    if metrics_status != dyncore.DYNVIEW_STATUS_OK {
...
large_op_visible_text :: #force_inline proc(
    ...
    return dyncore.text_for_command(buffer, cmd)
}
...
chars_per_row_for_style :: #force_inline proc(
    ...
    style: dyncore.Dynview_Text_Style) -> int {

    effective_advance := max(1.0, wrap_advance * max(0.5, style.wrap_scale))
    return dyncore.chars_per_text_row(panel_width - text_padding * 2, effective_advance)
}
