...
dynview_shaped_builder_enforces_exact_limits :: proc(t: ^testing.T) {
    ...
    glyphs := []core.Shaped_Glyph{{glyph_id = 1}}
    for index in 0..<core.DYNVIEW_MAX_SHAPED_RUNS {
    ...
    cache := new(core.Dynview_Compile_Cache)
    defer free(cache)
    testing.expect_value(t, shaped_builder_seal(
        &builder, cache, 1, core.DYNVIEW_MAX_SHAPED_RUNS, 7),
        core.Bounded_Builder_Status.Ok)
    testing.expect_value(t, len(cache^.shaped_runs), core.DYNVIEW_MAX_SHAPED_RUNS)
    testing.expect_value(t, len(cache^.shaped_glyphs),
        core.FONT_SHAPED_GLYPH_CAPACITY)
    testing.expect_value(t, shaped_builder_seal(
        &builder, cache, 1, core.DYNVIEW_MAX_SHAPED_RUNS, 7),
        core.Bounded_Builder_Status.Sealed)
...
dynview_shaped_builder_rejects_invalid_spans_and_generation :: proc(t: ^testing.T) {
    ...
    cache := new(core.Dynview_Compile_Cache)
    defer free(cache)
    glyph := []core.Shaped_Glyph{{glyph_id = 1}}

    builder: Dynview_Shaped_Builder
    testing.expect_value(t, shaped_builder_init(&builder, &arena, 7),
        core.Bounded_Builder_Status.Ok)
    testing.expect_value(t, shaped_builder_append(
        &builder, {0, .Primary, 1, 1, glyph, {}}), core.Bounded_Builder_Status.Ok)
    testing.expect_value(t, shaped_builder_seal(
        &builder, cache, 1, 1, 7), core.Bounded_Builder_Status.Invalid_Argument)
    testing.expect_value(t, len(cache^.shaped_runs), 0)

    core.arena_owner_reset(&arena)
    testing.expect_value(t, shaped_builder_init(&builder, &arena, 8),
        core.Bounded_Builder_Status.Ok)
    testing.expect_value(t, shaped_builder_append(
        &builder, {0, .Primary, 0, 1, glyph, {}}), core.Bounded_Builder_Status.Ok)
    testing.expect_value(t, shaped_builder_seal(
        &builder, cache, 1, 1, 9), core.Bounded_Builder_Status.Invalid_Argument)
...
dynview_shaped_builder_rejects_layout_and_glyph_spans :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, shaped_builder_seal(&builder, cache, 1, 1, 4),
        core.Bounded_Builder_Status.Invalid_Argument)
...
dynview_shaped_builder_allocation_failure_preserves_fallback :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, shaped_builder_init_with_allocator(
        &builder, allocator, 3), core.Bounded_Builder_Status.Ok)
    testing.expect_value(t, shaped_builder_append(&builder,
        {0, .Primary, 0, 1, []core.Shaped_Glyph{{glyph_id = 1}}, {}}),
        core.Bounded_Builder_Status.Allocation_Failed)
    testing.expect_value(t, len(cache^.shaped_runs), 0)
    testing.expect_value(t, len(cache^.shaped_glyphs), 0)
}
