...
math_shaping_test_shape :: proc(
    ...
    for codepoint in request.text {
        if glyph_count >= len(request.glyph_output) {
            return {}
        }
        request.glyph_output[glyph_count] = {
            glyph_id = u32(codepoint), x_advance = 640}
        glyph_count += 1
    }
...
math_shaping_test_service :: proc(
    ...
    return {
        ...
        base_pixel_size = 32,
        raster_ascent = 24,
        constants = math_shaping_test_constants(),
...
math_shaping_test_populate_recursive_fixture :: proc(runtime: ^app_core.Dynview_System) {
    ...
    cache^.math_commands[1] = {
        ...
        script_style_id = dyncore.DYNVIEW_STYLE_ITALIC, script_scale = 0.7,
        script_gap = 0.1}
    ...
    cache^.math_commands[4] = {
        ...
        script_style_id = dyncore.DYNVIEW_STYLE_ITALIC, script_scale = 0.6,
        accent_thickness = 0.05}
...
math_shaping_expect_cached_run :: proc(
    ...
    item, item_ok := math_program_item({
        cache = cache, buffer = &runtime^.command_buffer,
        cmd = command, font_size = 32, command_index = 0,
    })
...
math_shaping_populate_intrinsic_fixture :: proc(runtime: ^app_core.Dynview_System) {
    ...
    cache^.math_commands[0] = {
        ...
        text_offset = 0, text_len = 1}
    cache^.math_commands[1] = {
        kind = .Large_Op, style_id = dyncore.DYNVIEW_STYLE_ITALIC,
...
math_shaping_expect_recursive_accents :: proc(
    ...
    bar_item, bar_ok := math_program_item({
        cache = cache, buffer = &runtime^.command_buffer,
        cmd = cache^.math_commands[5], font_size = 32, command_index = 5,
        math_style = {.Text, false},
    })
    ...
    glyph_accent, glyph_accent_ok := math_program_item({
        cache = cache, buffer = &runtime^.command_buffer,
        cmd = cache^.math_commands[6], font_size = 32, command_index = 6,
        math_style = {.Text, false},
    })
...
math_shaping_expect_recursive_geometry :: proc(
    ...
    fraction_item, fraction_ok := math_shaping_fixture_item(runtime, 3)
    ...
    script_item, script_ok := math_shaping_fixture_item(runtime, 1)
...
dynview_math_shaping_propagates_recursive_metrics :: proc(t: ^testing.T) {
    ...
    radical_source := cache^.math_stretch_sources[4][0]
    ...
    math_shaping_expect_recursive_geometry(t, runtime)
