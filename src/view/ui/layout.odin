...
layout_terminal_rect :: proc(
    text_rect: geometrymodel.Rectangle) -> geometrymodel.Rectangle {
    return render_raylib.application_rectangle(clamp_non_negative_rect({
        text_rect.x + 6,
        text_rect.y + 6,
        text_rect.width - 12,
        text_rect.height - 12,
...
layout_landscape_regions :: proc(
    ...
    regions^.accordion_rect = render_raylib.application_rectangle(
        clamp_non_negative_rect({
        split_x + TREE_PANEL_PADDING, TREE_PANEL_PADDING,
        width - split_x - TREE_PANEL_PADDING * 2,
        height - TREE_PANEL_PADDING * 2}))
    regions^.text_rect = render_raylib.application_rectangle(
        clamp_non_negative_rect({
...
compute_ui_regions :: proc(
    ...
    case .Portrait:
        regions.world_rect = geometrymodel.Rectangle{0, 0, width, split_y}
        regions.accordion_rect = render_raylib.application_rectangle(
            clamp_non_negative_rect({
            TREE_PANEL_PADDING,
        ...
        view_layout := accordion_layout(
            render_raylib.rectangle(regions.accordion_rect), sections, .View)
        regions.text_rect = render_raylib.application_rectangle(view_layout.content)
        regions.terminal_rect = layout_terminal_rect(regions.text_rect)
    }
...
ui_rect_valid :: #force_inline proc(rect: geometrymodel.Rectangle) -> bool {
