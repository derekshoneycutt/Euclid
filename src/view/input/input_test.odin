...
// Verify reversible preedit is bounded, owner-routed, and never enters text events.
@(test)
input_test_composition_update_is_reversible :: proc(t: ^testing.T) {
    runtime: Input_Runtime
    owner := Input_Text_Owner{.Terminal_Editor, 4, 9}
    testing.expect(t, input_runtime_set_text_owner(&runtime, owner))
    testing.expect(t, input_runtime_inject_composition_start(&runtime, owner))
    testing.expect(t, input_runtime_inject_composition_update(
        &runtime, owner, "aλ", {1, 3}))
    frame := input_runtime_route_text_owner(&runtime, {}, owner, true)
    testing.expect(t, frame.composition.active)
    testing.expect_value(t, frame.composition.preedit, "aλ")
    testing.expect_value(t, frame.composition.selection,
        Input_Composition_Selection{1, 3})
    testing.expect_value(t, runtime.injected_event_count, 0)
}

// Verify malformed ranges reject atomically without replacing current preedit.
@(test)
input_test_composition_rejects_non_boundary_selection :: proc(t: ^testing.T) {
    runtime: Input_Runtime
    owner := Input_Text_Owner{.Terminal_Editor, 1, 2}
    input_runtime_set_text_owner(&runtime, owner)
    input_runtime_inject_composition_start(&runtime, owner)
    input_runtime_inject_composition_update(&runtime, owner, "old", {0, 3})
    testing.expect(t, !input_runtime_inject_composition_update(
        &runtime, owner, "λ", {1, 2}))
    frame := input_runtime_route_text_owner(&runtime, {}, owner, true)
    testing.expect_value(t, frame.composition.preedit, "old")
    testing.expect_value(t, runtime.composition_rejection_count, u64(1))
}

// Verify commit emits ordinary synthetic text once and clears reversible state.
@(test)
input_test_composition_commit_uses_text_event_route :: proc(t: ^testing.T) {
    runtime: Input_Runtime
    owner := Input_Text_Owner{.Terminal_Editor, 3, 7}
    input_runtime_set_text_owner(&runtime, owner)
    input_runtime_inject_composition_start(&runtime, owner)
    input_runtime_inject_composition_update(&runtime, owner, "λ", {0, 2})
    testing.expect(t, input_runtime_inject_composition_commit(
        &runtime, owner, "λx"))
    testing.expect(t, !runtime.composition_active)
    testing.expect(t, input_runtime_begin_frame(&runtime))
    events := input_runtime_events(&runtime)
    testing.expect_value(t, len(events), 2)
    testing.expect_value(t, events[0].kind, Input_Event_Kind.Text)
    testing.expect_value(t, events[0].codepoint, rune('λ'))
    testing.expect_value(t, events[1].codepoint, rune('x'))
    testing.expect_value(t, runtime.injected_event_count, 0)
}

// Verify explicit cancellation removes preedit without producing committed text.
@(test)
input_test_composition_cancel_discards_preedit :: proc(t: ^testing.T) {
    runtime: Input_Runtime
    owner := Input_Text_Owner{.Terminal_Editor, 2, 6}
    input_runtime_set_text_owner(&runtime, owner)
    input_runtime_inject_composition_start(&runtime, owner)
    input_runtime_inject_composition_update(&runtime, owner, "discard", {0, 0})
    testing.expect(t, input_runtime_inject_composition_cancel(&runtime, owner))
    testing.expect(t, !runtime.composition_active)
    testing.expect_value(t, runtime.composition_byte_count, 0)
    testing.expect_value(t, runtime.injected_event_count, 0)
}

// Verify focus loss and generation replacement cancel stale preedit.
@(test)
input_test_composition_cancels_on_focus_or_owner_change :: proc(t: ^testing.T) {
    runtime: Input_Runtime
    first := Input_Text_Owner{.Terminal_Editor, 1, 4}
    second := Input_Text_Owner{.Terminal_Editor, 1, 5}
    input_runtime_set_text_owner(&runtime, first)
    input_runtime_inject_composition_start(&runtime, first)
    input_runtime_inject_composition_update(&runtime, first, "pending", {7, 7})
    frame := input_runtime_route_text_owner(&runtime, {}, first, false)
    testing.expect(t, !frame.composition.active)
    testing.expect(t, !runtime.composition_active)
    testing.expect_value(t, runtime.composition_stale_cancel_count, u64(1))
    input_runtime_set_text_owner(&runtime, second)
    testing.expect(t, !input_runtime_inject_composition_start(&runtime, first))
    testing.expect_value(t, runtime.composition_rejection_count, u64(1))
}
