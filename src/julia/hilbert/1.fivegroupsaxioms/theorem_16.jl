module HilbertChapterOneTheorem16
...
const SideBCRadius = Float32(sqrt(
    (PointC[1] - PointB[1])^2 + (PointC[2] - PointB[2])^2))
const SideACRadius = Float32(sqrt(
    (PointC[1] - PointA[1])^2 + (PointC[2] - PointA[2])^2))
const SideAPrimeBPrimeRadius = Float32(sqrt(
    (PointBPrime[1] - PointAPrime[1])^2 +
    (PointBPrime[2] - PointAPrime[2])^2))
const SideBPrimeCPrimeRadius = Float32(sqrt(
    (PointCPrime[1] - PointBPrime[1])^2 +
    (PointCPrime[2] - PointBPrime[2])^2))
const SideAPrimeCPrimeRadius = Float32(sqrt(
    (PointCPrime[1] - PointAPrime[1])^2 +
    (PointCPrime[2] - PointAPrime[2])^2))
...
const PhaseArcAPrimeToBForBC = 14f0
const PhaseDragBCForward = 15f0
const PhaseDragBCBack = 16f0
const PhaseArcBToBPrimeForPrimeBC = 17f0
const PhaseDragBPrimeCPrimeForward = 18f0
const PhaseDragBPrimeCPrimeBack = 19f0
const PhaseArcBPrimeToAForAC = 20f0
const PhaseDragACForward = 21f0
const PhaseDragACBack = 22f0
const PhaseArcAToAPrimeForPrimeAC = 23f0
const PhaseDragAPrimeCPrimeForward = 24f0
const PhaseDragAPrimeCPrimeBack = 25f0
const PhasePenRiseBeforeCompass = 26f0

const PhaseCompassDescendA = 27f0
const PhaseHighlightAngleAForward = 28f0
const PhaseHighlightAngleABack = 29f0
const PhaseCompassArcAToAPrime = 30f0
const PhaseHighlightAngleAPrimeForward = 31f0
const PhaseHighlightAngleAPrimeBack = 32f0
const PhaseCompassArcAPrimeToB = 33f0
const PhaseHighlightAngleBForward = 34f0
const PhaseHighlightAngleBBack = 35f0
const PhaseCompassArcBToBPrime = 36f0
const PhaseHighlightAngleBPrimeForward = 37f0
const PhaseHighlightAngleBPrimeBack = 38f0
const PhaseCompassArcBPrimeToC = 39f0
const PhaseHighlightAngleCForward = 40f0
const PhaseHighlightAngleCBack = 41f0
const PhaseCompassArcCToCPrime = 42f0
const PhaseHighlightAngleCPrimeForward = 43f0
const PhaseHighlightAngleCPrimeBack = 44f0
const PhaseCompassRise = 45f0
const PhaseFinalHold = 46f0
...
function reset_cycle_state(state_ptr::Ptr{Cvoid})
    ...
    edge_c_a_host_id = Integer(OdinJuliaBridge.get_animation_meta(state_ptr, MetaEdgeCAHostId))
    edge_c_a_joint2_id = Integer(OdinJuliaBridge.get_animation_meta(state_ptr, MetaEdgeCAJoint2Id))
    ...
    OdinJuliaBridge.set_point_position(state_ptr, edge_c_prime_a_prime_joint2_id, EdgeCPrimeAPrimeStart)
...
function initialize(state_ptr::Ptr{Cvoid})
    ...
    edge_c_a = OdinJuliaBridge.create_new_line(state_ptr, EdgeCAStart, EdgeCAStart, EdgeCAColor, 0f0)
    ...
    OdinJuliaBridge.set_animation_meta(state_ptr, MetaEdgeCAHostId, Float32(edge_c_a.host_id))
    OdinJuliaBridge.set_animation_meta(state_ptr, MetaEdgeCAJoint1Id, Float32(edge_c_a.joint1_id))
    OdinJuliaBridge.set_animation_meta(state_ptr, MetaEdgeCAJoint2Id, Float32(edge_c_a.joint2_id))
    ...
    OdinJuliaBridge.set_animation_meta(
        state_ptr, MetaEdgeCPrimeAPrimeHostId, Float32(edge_c_prime_a_prime.host_id))
    OdinJuliaBridge.set_animation_meta(
        state_ptr, MetaEdgeCPrimeAPrimeJoint1Id, Float32(edge_c_prime_a_prime.joint1_id))
    OdinJuliaBridge.set_animation_meta(
        state_ptr, MetaEdgeCPrimeAPrimeJoint2Id, Float32(edge_c_prime_a_prime.joint2_id))
...
function loop(state_ptr::Ptr{Cvoid}, dt::Float32)
    ...
    edge_b_c_joint2_id = Integer(OdinJuliaBridge.get_animation_meta(state_ptr, MetaEdgeBCJoint2Id))
    edge_c_a_host_id = Integer(OdinJuliaBridge.get_animation_meta(state_ptr, MetaEdgeCAHostId))
    edge_c_a_joint1_id = Integer(OdinJuliaBridge.get_animation_meta(state_ptr, MetaEdgeCAJoint1Id))
    edge_c_a_joint2_id = Integer(OdinJuliaBridge.get_animation_meta(state_ptr, MetaEdgeCAJoint2Id))
    ...
    elseif phase == PhaseDrawAB
        EuclidAnimations.animate_draw_line(
            ...
            EdgeBrush, EdgeABColor, edge_a_b_host_id, edge_a_b_joint1_id, edge_a_b_joint2_id)
        timer += dt
    ...
    elseif phase == PhaseDrawBC
        EuclidAnimations.animate_draw_line(
            ...
            EdgeBrush, EdgeBCColor, edge_b_c_host_id, edge_b_c_joint1_id, edge_b_c_joint2_id)
        timer += dt
        if timer >= DrawEdgeDuration
    ...
    elseif phase == PhaseDrawCA
        EuclidAnimations.animate_draw_line(
            ...
            EdgeBrush, EdgeCAColor, edge_c_a_host_id, edge_c_a_joint1_id, edge_c_a_joint2_id)
        timer += dt
        if timer >= DrawEdgeDuration
    ...
    elseif phase == PhaseDrawCPrimeAPrime
        EuclidAnimations.animate_draw_line(
            ...
            edge_c_prime_a_prime_host_id, edge_c_prime_a_prime_joint1_id, edge_c_prime_a_prime_joint2_id)
        timer += dt
        if timer >= DrawEdgeDuration
    ...
    elseif phase == PhaseDragBPrimeCPrimeBack
        ...
        timer += dt
        if timer >= DragDuration
            phase = PhaseArcBPrimeToAForAC
            timer = 0f0
        end
    elseif phase == PhaseArcBPrimeToAForAC
        EuclidAnimations.animate_pen_arcmove(
    ...
    elseif phase == PhasePenRiseBeforeCompass
        ...
        if timer >= PenLiftDuration
            OdinJuliaBridge.hide_pen(state_ptr)
            phase = PhaseCompassDescendA
            timer = 0f0
        end

    elseif phase == PhaseCompassDescendA
