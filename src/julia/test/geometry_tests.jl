...
@testset "circle_line_intersections_xy" begin
    @testset "two intersections" begin
        ...
        points = sorted_points_xy(
            circle_line_intersections_xy(line_a, line_b, center, radius))
    ...
    @testset "invalid arguments" begin
        center = Float32[0f0, 0f0, 0f0]
        @test_throws ArgumentError circle_line_intersections_xy(
            Float32[0f0, 0f0, 0f0], Float32[0f0, 0f0, 0f0], center, 1f0)
        @test_throws ArgumentError circle_line_intersections_xy(
            Float32[0f0], Float32[1f0, 0f0], center, 1f0)
        @test_throws ArgumentError circle_line_intersections_xy(
            Float32[0f0, 0f0], Float32[1f0, 0f0], center, -1f0)
    end
end
...
@testset "circle_circle_intersections_xy" begin
    @testset "two intersections" begin
        ...
        points = sorted_points_xy(
            circle_circle_intersections_xy(center1, 1f0, center2, 1f0))
        @test length(points) == 2
    ...
    @testset "no intersection" begin
        @test isempty(circle_circle_intersections_xy(
            Float32[0f0, 0f0, 0f0], 1f0, Float32[4f0, 0f0, 0f0], 1f0))
        @test isempty(circle_circle_intersections_xy(
            Float32[0f0, 0f0, 0f0], 5f0, Float32[1f0, 0f0, 0f0], 1f0))
        @test isempty(circle_circle_intersections_xy(
            Float32[0f0, 0f0, 0f0], 1f0, Float32[0f0, 0f0, 0f0], 1f0))
    end

    @testset "invalid arguments" begin
        @test_throws ArgumentError circle_circle_intersections_xy(
            Float32[0f0, 0f0], -1f0, Float32[1f0, 0f0], 1f0)
        @test_throws ArgumentError circle_circle_intersections_xy(
            Float32[0f0, 0f0], 1f0, Float32[1f0, 0f0], -1f0)
    end
end
...
@testset "line_intersection_3d" begin
    ...
    @testset "invalid arguments" begin
        @test_throws ArgumentError line_intersection_3d(
            Float32[0f0, 0f0], Float32[1f0, 1f0, 1f0],
            Float32[0f0, 1f0, 0f0], Float32[1f0, 0f0, 0f0])
        @test_throws ArgumentError line_intersection_3d(
            Float32[0f0, 0f0, 0f0], Float32[0f0, 0f0, 0f0],
            Float32[0f0, 1f0, 0f0], Float32[1f0, 0f0, 0f0])
    end
end
