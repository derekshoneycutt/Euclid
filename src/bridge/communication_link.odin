...
communication_link_init :: proc(
    link: ^bridgemodel.Communication_Link($T), channel_capacity: int, pool_size: int,
...
communication_link_destroy :: proc(link: ^bridgemodel.Communication_Link($T)) {
...
communication_link_alloc :: proc(
    link: ^bridgemodel.Communication_Link($T)) -> (^T, runtime.Allocator_Error) {
...
communication_link_alloc_bytes :: proc(
    link: ^bridgemodel.Communication_Link($T), count: int) ->
...
communication_link_free_bytes :: proc(
    link: ^bridgemodel.Communication_Link($T), bytes: []u8) {
    delete(bytes, tlsf.allocator(&link.pool))
}

// Reclaim one envelope that the producer has not transferred or has received back.
communication_link_free :: proc(link: ^bridgemodel.Communication_Link($T), message: ^T) {
...
communication_link_try_send :: proc(
    link: ^bridgemodel.Communication_Link($T), message: ^T) -> bool {
...
communication_link_recv :: proc(
    link: ^bridgemodel.Communication_Link($T)) -> (^T, bool) {
...
communication_link_drain_returns :: proc(
    link: ^bridgemodel.Communication_Link($T)) -> int {
    count := 0
    for {
